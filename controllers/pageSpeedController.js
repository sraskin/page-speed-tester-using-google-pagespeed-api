...
exports.checkPerformance = async (req, res) => {
    ...
    if (!testUrl) {
        return res.status(400).json({
            message:
                'URL is required in testUrl query parameter. Example: /check-performance?testUrl=example.com'
        });
    }
    ...
    await getPerformanceScore('MOBILE', testUrl).then(async (res) => {
        ...
        const accessibilityScore =
            res.lighthouseResult.categories.accessibility.score * 100;
        const bestPracticesScore =
            res.lighthouseResult.categories['best-practices'].score * 100;
        const seoScore = res.lighthouseResult.categories.seo.score * 100;
        const pwaScore =
            res?.lighthouseResult?.categories?.pwa?.score * 100 ?? null;
        ...
        const loadingExperience = {
            CUMULATIVE_LAYOUT_SHIFT_SCORE: {
                percentile:
                    res.loadingExperience.metrics.CUMULATIVE_LAYOUT_SHIFT_SCORE
                        .percentile /
                        100 +
                    '%',
                category:
            ...
            TIME_TO_FIRST_BYTE: {
                percentile:
                    res.loadingExperience.metrics
                        .EXPERIMENTAL_TIME_TO_FIRST_BYTE.percentile /
                        1000 +
                    's',
                category:
                    res.loadingExperience.metrics
                        .EXPERIMENTAL_TIME_TO_FIRST_BYTE.category,
                explanation:
                    'Good (≤ 0.8 s), Needs Improvement (0.8 - 1.8 s), Poor (> 1.8 s)'
            },
            FIRST_CONTENTFUL_PAINT: {
                percentile:
                    res.loadingExperience.metrics.FIRST_CONTENTFUL_PAINT_MS
                        .percentile + 's',
                category:
                    res.loadingExperience.metrics.FIRST_CONTENTFUL_PAINT_MS
                        .category,
            ...
            FIRST_INPUT_DELAY: {
                percentile:
                    res.loadingExperience.metrics.FIRST_INPUT_DELAY_MS
                        .percentile + 'ms',
                category:
                    res.loadingExperience.metrics.FIRST_INPUT_DELAY_MS.category,
                explanation:
                    'Good (≤ 100 ms), Needs Improvement (100 - 300 ms), Poor (> 300 ms)'
            },
            INTERACTION_TO_NEXT_PAINT: {
                percentile:
                    res.loadingExperience.metrics.INTERACTION_TO_NEXT_PAINT
                        .percentile + 'ms',
                category:
                    res.loadingExperience.metrics.INTERACTION_TO_NEXT_PAINT
                        .category,
            ...
            LARGEST_CONTENTFUL_PAINT: {
                percentile:
                    res.loadingExperience.metrics.LARGEST_CONTENTFUL_PAINT_MS
                        .percentile /
                        1000 +
                    's',
                category:
    ...
    await getPerformanceScore('DESKTOP', testUrl).then(async (res) => {
        ...
        const performanceScore =
            res?.lighthouseResult?.categories?.performance?.score * 100 ?? null;
        const accessibilityScore =
            res?.lighthouseResult?.categories?.accessibility?.score * 100 ??
            null;
        const bestPracticesScore =
            ...
            null;
        const seoScore =
            res?.lighthouseResult?.categories?.seo?.score * 100 ?? null;
        const pwaScore =
            res?.lighthouseResult?.categories?.pwa?.score * 100 ?? null;
        const entities = res.lighthouseResult.entities;
